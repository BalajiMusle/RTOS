...
int main()
{
	...
	// *** init sem count = 0
	//	flag=S_FIFO --> wake up tasks as per their arrival
	//	flag=S_PRIO --> wake up tasks as per their priority (high prio first)
	//ret = rt_sem_create(&s, "SEM1", 0, S_FIFO);
	ret = rt_sem_create(&s, "SEM1", 0, S_PRIO);
	...
	// *** perform semaphore v operation 5 times -- resume all tasks -- at once
	for(i=0; i<5; i++)
		rt_sem_v(&s);
